...
//       if (res.data.status === "Success") {
//         setDetails(res.data.data); // Make sure to set data, not res.data
//       } else {
//         ErrorToast(res.data.message || "Failed to fetch purchase details");
//       }
//     } catch (error) {
...
//               <img src={rafidSeeds} height={70} width={70} alt="rafidseeds" />
//               <div>
//                 <h1>{businessDetails?.name || "Business Name"}</h1>
//                 <p className="text-sm">
...
//               <p className="font-semibold mb-2">Supplier</p>
//               <p>{details?.Supplier?.name || "N/A"}</p>
//               <p>{details?.Supplier?.mobile || "N/A"}</p>
//               <p>{details?.Supplier?.address || "N/A"}</p>
//             </div>
//           </div>
...
//               Purchase Invoice
//             </h1>
//             <p>Invoice No: {details?.PurchaseSummary?.Reference || id}</p>
//             <p>Date: {formatDate(details?.PurchaseSummary?.Date)}</p>
//             <p>{businessDetails?.mobile || "N/A"}</p>
//             <p className="break-words max-w-[110px] lg:w-full">
//               {businessDetails?.email || "N/A"}
//             </p>
//             <p>{businessDetails?.website || "N/A"}</p>
//           </div>
//         </div>
...
//                 <th className="global_print_th">Qty</th>
//                 <th className="global_print_th">Unit Cost</th>
//                 <th className="global_print_th">DP</th>
//                 <th className="global_print_th">MRP</th>
...
//                 <tr key={p.id || i}>
//                   <td className="global_td">{i + 1}</td>
//                   <td className="global_td">{p.name || "N/A"}</td>
//                   <td className="global_td">{p.quantity || 0}</td>
//                   <td className="global_td">{p.unitCost || 0}</td>
//                   <td className="global_td">{p.dp || 0}</td>
//                   <td className="global_td">{p.mrp || 0}</td>
//                   <td className="global_td">{p.warranty || 0}</td>
...
//             <p className="flex justify-between p-2">
//               <strong>Total:</strong>
//               <strong>{details?.PurchaseSummary?.total || 0} Taka</strong>
//             </p>
...
//             <p className="bg-green-300 dark:text-black flex justify-between px-2 py-2">
//               <strong>Grand Total:</strong>
//               {details?.PurchaseSummary?.grandTotal || 0} Taka
//             </p>
//             <p className="flex justify-between p-2">
//               <strong>Paid:</strong>
//               {details?.PurchaseSummary?.paid || 0} Taka
//             </p>
//             <p className="flex justify-between p-2">
//               <strong>Due:</strong>
//               {details?.PurchaseSummary?.dueAmount || 0} Taka
//             </p>
//           </div>
//         </div>
//       </div>
...
const PurchaseDetails = () => {
  ...
  const getPurchaseDetailsByID = async (id) => {
    ...
    try {
      ...
      if (res.data.status === "Success") {
        setDetails(res.data.data); // ✅ data directly
      } else {
        ErrorToast(res.data.message || "Failed to fetch details");
      }
    } catch (error) {
      ErrorToast(error.message || "Something went wrong");
      console.error(error);
